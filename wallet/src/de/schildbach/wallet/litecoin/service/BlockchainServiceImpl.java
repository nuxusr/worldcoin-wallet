...
public class BlockchainServiceImpl extends android.app.Service implements BlockchainService
{
	...
	private static final String TAG = "Worldcoin"+BlockchainServiceImpl.class.getSimpleName();
	...
	private void notifyCoinsReceived(final Address from, final BigInteger amount)
	{
		...
		final int precision = Integer.parseInt(prefs.getString(Constants.PREFS_KEY_WDC_PRECISION, Integer.toString(Constants.WDC_PRECISION)));
		...
		private void check()
		{
			...
			if (hasEverything && peerGroup == null)
			{
                ...
                try {
                    ...
                    Log.e("Worldcoin", "There's no method: " + e.getLocalizedMessage());
                    Log.e("Worldcoin", "Worldcoinj issue.  We're going to ignore this for now and just try and return nicely.");
                    return;
                }
	...
	public void notifyWidgets()
	{
        ...
        try {
            ...
            Log.e("Worldcoin", "App server appears dead - Runtime Exception when running getAppWidgetIds.  Returning..");
            return;
        }
